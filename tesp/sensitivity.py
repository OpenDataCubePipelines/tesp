...
def combine_parts(linear, shift):
    ...
    """
    return Affine(linear[0, 0], linear[0, 1], shift[0], linear[1, 0], linear[1, 1], shift[1])
...
def combine_affine(left_affine, right_affine):
    ...
    steps = numpy.linalg.solve(linear_part(left_affine), translation)
    shift = numpy.array(left_affine * numpy.where(steps > 0, 0, steps))
...
def merge_datasets(left_product, right_product, target_product, band_name):
    ...
    assert numpy.allclose(linear_part(left_affine), linear_part(right_affine))
    ...
    target_shape = (int(max(left_end[0], right_end[0])), int(max(left_end[1], right_end[1])))

    target_ds = target_product.create_dataset(band_name, shape=target_shape, dtype='int32', chunks=(1024, 1024))
    target_ds[:] = 0
    target_count = target_product.create_dataset(band_name + "_pixelcount",
                                                 shape=target_shape, dtype='int32', chunks=(1024, 1024))
    target_count[:] = 0
    ...
    def add_image(src_affine, src_product):
        ...
        for coord in ul:
            assert abs(coord - numpy.around(coord)) < src_affine.precision

        for coord in lr:
            assert abs(coord - numpy.around(coord)) < src_affine.precision

        ul = [int(numpy.around(coord)) for coord in ul]
        lr = [int(numpy.around(coord)) for coord in lr]
